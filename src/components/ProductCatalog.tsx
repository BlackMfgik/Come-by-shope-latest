...
import { apiGetProducts, apiGetProduct, apiCreateProduct, apiUpdateProduct, apiDeleteProduct } from '../api';
...
export default function ProductCatalog({ searchQuery = '', category }: Props) {
  ...
  // Admin form state
  const [adminForm, setAdminForm] = useState({ name: '', description: '', weight: '', price: '', imageName: '' });
  ...
  async function handleAdminSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert(err.message || 'Помилка при збереженні товару');
    }
  }
  ...
  async function handleEdit(id: number) {
    ...
    } catch { alert('Не вдалося завантажити товар'); }
  }
  ...
  async function handleDelete(id: number) {
    ...
    try {
      ...
      await loadProducts();
    } catch { alert('Не вдалося видалити товар'); }
  }
  ...
  return (
    <>
      {isAdmin && (
        <section id="admin-panel" className="admin-panel">
          <form id="admin-product-form" onSubmit={handleAdminSubmit}>
            ...
            {editingId != null && (
              <button type="button" onClick={() => { setEditingId(null); setAdminForm({ name: '', description: '', weight: '', price: '', imageName: '' }); }}
                style={{ marginLeft: 8 }}>
                Скасувати
              </button>
            )}
      ...
      <section className="catalog" id="catalog">
        {loading && <p style={{ textAlign: 'center', opacity: 0.7 }}>Завантаження...</p>}
        {error && <p className="error-wrap">{error}</p>}
        {!loading && !error && filtered.length === 0 && (
          <p style={{ textAlign: 'center', opacity: 0.7 }}>Товарів поки немає</p>
        )}
        {filtered.map(p => (
          <div className="product-card" data-id={p.id} key={p.id}>
            ...
            <div className="product-info">
              ...
              <div className="product-bottom">
                ...
                <button
                  className="add-btn"
                  onClick={() => addItem(p.name, p.price, p.image || '/images/no-image.png', p.description || '', p.id)}
                >
                  <span>+</span> Додати
                </button>
              </div>
              {isAdmin && (
                <div className="admin-controls admin-only">
                  <button className="admin-controls-btn admin-edit-btn" type="button" title="Редагувати" onClick={() => handleEdit(p.id)}>
                    <img src="/images/edit-button.png" alt="edit" />
                  </button>
                  <button className="admin-controls-btn admin-delete-btn" type="button" title="Видалити" onClick={() => handleDelete(p.id)}>
