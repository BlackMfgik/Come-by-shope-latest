...
export default function OrderDetailsModal({ order, onClose }: Props) {
  const status = STATUS_MAP[order.status] ?? { label: order.status, color: "var(--text-2)" };
    ...
    >
      <div className="modal-content">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <h3 id="modal-title" style={{ margin: 0 }}>Замовлення #{order.id}</h3>
        </div>
        <p style={{ color: "var(--text-2)", fontSize: "0.83rem", margin: "4px 0 16px" }}>{date}</p>
        ...
        <div className="modal-buttons" style={{ marginTop: 8 }}>
          <button className="order-btn secondary" onClick={onClose}>Закрити</button>
        </div>
      </div>
    </div>
  );
